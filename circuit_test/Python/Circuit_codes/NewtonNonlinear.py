...
def Newton_system(x_guess, eps):
    ...
    while error > eps and iteration_counter < 5000:
        F_value = np.matmul(J(x_old),x_old)-RHS # "Generic" expanded Jacobian input
...
#     """
#     x_old = x_guess
#     F_x = np.matmul(J(x_old),x_old)
#     error = 9e9
#     iteration_counter = 0
#     while error > eps and iteration_counter < 5000:
#         delta = np.linalg.solve(J(x_old), F_x)
#         x_new = x_old - delta
#         F_x = np.matmul(J(x_old),x_old)
#         J(x_old) = np.matmul(J(x_new),x_new)
#         error = np.max(np.abs(x_new-x_old))
#         # error = np.linalg.norm(F_x)
#         # error = np.max(np.abs(delta - x_new))
#         x_old = x_new
#         iteration_counter += 1
#         print(-x_new,iteration_counter, error)
...
st = tm.time()
ans, n = Newton_system(x_guess = np.array([1, 0.6823, -0.003]), eps=1e-20)
et = tm.time()
diff = et - st
