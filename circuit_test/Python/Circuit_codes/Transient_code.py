...
def fet_assigner(number, node_vd, node_vg, node_vs, node_vb, solution, LHS, RHS,  mode):
    ...
    if(node_vd == 0):
        LHS = R_assigner(0,1+4+add,0,LHS) # RD
        node_vd = 1
        vd = 0
    else:
        LHS = R_assigner(node_vd+add,node_vd+4+add,cond(1),LHS) # RD
        vd = solution[node_vd-1+add,0]
        
    if(node_vg == 0):
        vg = 0
    else:
        vg = solution[node_vg-1+add,0]
    
    if(node_vs == 0):
        vs = 0
    else:
        vs = solution[node_vs-1+add,0]
        
    if(node_vb == 0):
        vb = 0
    else:
        vb = solution[node_vb-1+add,0]
    # the other settings for fet model based on the large signal analysis
    LHS = R_assigner(node_vg+add,node_vd+5+add,cond(1),LHS) # RG
    LHS = R_assigner(node_vs+add,node_vd+7+add,cond(5e3),LHS) # RS
    LHS = R_assigner(node_vb+add,node_vd+10+add,cond(1),LHS) # RB
    LHS = R_assigner(node_vd+12+add,node_vd+13+add,cond(1e6),LHS) # RDS
    LHS, RHS = Diode_assigner(node_vd+11+add,node_vd+12+add,2.7e-9,0.05,0,h,LHS,RHS,solution) # Diode BD
    LHS, RHS = Diode_assigner(node_vd+9+add,node_vd+13+add,2.7e-9,0.05,0,h,LHS,RHS,solution) # Diode BS
    LHS, RHS = C_assigner(node_vd+11+add,node_vd+4+add,0,LHS,RHS,solution,h,mode) # Capacitor BD
    LHS, RHS = C_assigner(node_vd+4+add,node_vd+5+add,0,LHS,RHS,solution,h,mode) # Capacitor GD
    LHS, RHS = C_assigner(node_vd+6+add,node_vd+7+add,0,LHS,RHS,solution,h,mode) # Capacitor GBO_1
    LHS, RHS = C_assigner(node_vd+6+add,node_vb+8+add,0,LHS,RHS,solution,h,mode) # Capacitor GBO_2
    LHS, RHS = C_assigner(node_vb+7+add,node_vb+8+add,0,LHS,RHS,solution,h,mode) # Capacitor BSO
    ...
    K = 1/2*(mCox)*(W/L)
    vsb = vs - vb
    vt = vt0 + gamma*((np.sqrt(phi+vsb)-np.sqrt(phi))) # already taking into account the body effect of MOSFETs
    vgs = vg - vs
    vds = vd - vs
    if (vgs>vt) and (vds <= vgs-vt):
        id = K*(vgs - vt)*vds-np.power(vds,2)*(1+LAMBDA*vds)
    elif (vgs>vt) and (vds >= vgs-vt):
        id = K * np.power((vgs - vt),2) * (1+LAMBDA*vds)
    else:
        id=0
    ...
    if(node_vd == 0):
        New_RHS = Is_assigner(1+12+add,1+13+add,id,RHS)
    else:
        New_RHS = Is_assigner(node_vd + 12+add, node_vd + 13+add,id,RHS)
    New_LHS = LHS
    print(New_RHS,"\n",New_LHS)
    return New_LHS,New_RHS
...
def OPanalysis_system(LHS,RHS):
    ...
    while error > eps and iteration_counter < 50:
        LHS, RHS = fet_assigner(1, 1, 0, 3, 3, solution, LHS, RHS, 0)
...
that contains only zeros which will then be occupied with values from the components.
    ...
    - each FET transistor adds 14 nodes
'''
T_nodes = 14
...
LHS, RHS, temp = Vs_assigner(5, 1, 0, LHS, RHS)
LHS, RHS, temp = Vs_assigner(5, 0, 3, LHS, RHS)
# LHS, RHS, Vs_locate = Vs_assigner(Vs[0], 1, 0, LHS, RHS)

# OP analysis for the initial conditions for the transient simulation
LHS, RHS, init = OPanalysis_system(LHS,RHS)
print("LHS:",LHS)
print("RHS:",RHS)
